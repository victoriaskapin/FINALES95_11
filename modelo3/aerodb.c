...
aerodb_t *aerodb_leer(const char *ruta){
    ...
    while(fread(&aux,sizeof(aerop_t),1,fp)){
        if(aerodb_agregar(a,aux.designacion,aux.nombre,aux.ciudad,aux.pais,aux.latitud,aux.longitud));      
    }
...
aerop_t* buscar_aeropuerto_recursivo(nodo_t *a, const char *designacion,size_t start, size_t end,size_t m_prev){
    if(a == NULL)
        return NULL;
    ...
    if(comparacion > 0){
        ...
        return buscar_aeropuerto_recursivo(a,designacion,start,end,mid);
    }
    else{
        start = mid+1; 
        return buscar_aeropuerto_recursivo(a,designacion,start,end,mid);
...
aerop_t* buscar_aeropuerto_binario(const aerodb_t *a, const char *designacion){
    ...
    mid = end+1;
    aerop_t * aero = buscar_aeropuerto_recursivo(a->prim,designacion,start,end,mid); 
    if (aero == NULL)    
     return NULL; 
...
//EJERCICIO 4 
double distancia (double xlat1,double xlat2,double ylon1,double ylon2){
    double lat = (xlat1-xlat2);
    double lon = (ylon1-ylon2);
    return ((lat*lat)+(lon*lon));
}//devuelve la distancia cuadrada entre 2 puntos. 
aerop_t* aero_proximo(aerodb_t *a,double xlat,double xlon){
    aerop_t *min;
    nodo_t* aux;
    ...
    //primera iteracion 
    double daux;
    double distmin = distancia(xlat,aux->elem->latitud,xlon,aux->elem->longitud);
    while(aux != NULL){
        daux = distancia(xlat,aux->elem->latitud,xlon,aux->elem->longitud);
        if(distmin > daux){
            min = aux->elem; 
            distmin = daux;
        }
...
bool aerodb_mas_cercano(aerodb_t *a, double xlat, double xlon, char **nombre, char **ciudad, char **pais, double *lat, double *lon){
    if(a == NULL)
        return false;
    aerop_t *aero = aero_proximo(a,xlat,xlon); 
    if(aero == NULL ) 
        return false; 
    *nombre = aero->nombre; 
    *ciudad = aero->ciudad; 
